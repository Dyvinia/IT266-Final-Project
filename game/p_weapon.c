...
void ApexBulletPattern_Fire(edict_t* ent, vec3_t g_offset, int damage, qboolean hyper, int effect, float x, float y, int kick, int speed)
{
...
{
	ApexBulletAuto_Fire(ent, vec3_origin, 5, false, NULL, 8, 0, 0.7, true);
}

void Weapon_Flatline_Fire(edict_t* ent)
{
	ApexBulletAuto_Fire(ent, vec3_origin, 8, false, NULL, 1, 1, 1.1, true);
}
...
void Weapon_Eva_Fire(edict_t* ent)
{
	float coords[8][2] = {
		{-7.0, 7.0},
		{0.0, 11.0},
	...
	for (int i = 0; i < 8; i++) {
		ApexBulletPattern_Fire(ent, vec3_origin, 4, false, NULL, coords[i][0] * 0.003, coords[i][1] * 0.003, 1, 1500);
	}
	ent->client->ps.gunframe++;
}
...
void Weapon_TTake_Fire(edict_t* ent)
{
	float coords[3][2] = {
		{0.0, 0.0},
		{-3.0, 0.0},
...
void Weapon_Havoc(edict_t* ent)
{
	...
	Weapon_Generic(ent, 0, 5, 45, 49, pause_frames, fire_frames, Weapon_R301_Fire);
}
...
{
	static int    pause_frames[] = { 9, 29, 42, 57, 0 };
	static int    fire_frames[] = { 7, 0 };
...
void Weapon_TTake(edict_t* ent)
{
	...
	Weapon_Generic(ent, 4, 12, 45, 49, pause_frames, fire_frames, Weapon_TTake_Fire);
}
